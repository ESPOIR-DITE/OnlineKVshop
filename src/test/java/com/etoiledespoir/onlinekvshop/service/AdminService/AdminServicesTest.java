package com.etoiledespoir.onlinekvshop.service.AdminService;

public class AdminServicesTest{}/** {
    private AdminServices adminRepository= AdminServiceFact.getAdminService();
    Admin admin = AdminFactory.buildAdmin("espoir",1001,"esp","203838");

    @Before
    public void setUp() throws Exception {
    }

    @Test
    public void getadminService() {

    }

    @Test
    public void creat() {
        System.out.println(adminRepository.creat(admin).toString());
    }

    @Test
    public void delete() {
        System.out.println(adminRepository.delete("1002").toString());
    }

    @Test
    public void update() {
        System.out.println(adminRepository.Update(admin).toString());
    }

    @Test
    public void read() {
    }

    @Test
    public void readAll() {
    }
}*/